...
async function sessdl() {
  try {
    ...
    const base64Data = sess.SESSION_ID.split("ROKEY_MD@")[1];
    if (!base64Data) {
...
async function connectToWA() {
  ...
  //===========================
  console.log("🔥 ROKEY_BOT is starting...");
  ...
  conn.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      let up = `
          @**ROKEY_MD Connected Successfully!**
          
                     **Welcome to Rokey-bot!**  
      ...
      conn.sendMessage(conn.user.id, {
        text: up,
        contextInfo: {
          mentionedJid: ["94740571366@s.whatsapp.net"], // specify mentioned JID(s) if any
          groupMentions: [],
          forwardingScore: 999,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
            newsletterJid: "@newsletter",
            newsletterName: "ROKEY_MD",
            serverMessageId: 999,
          },
          externalAdReply: {
            title: "ROKEY_MD",
